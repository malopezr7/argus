...
Status: **`v0.2.4`** — installable from npm as
...
- [x] Build pipeline (`pnpm build` → `scripts/build-package.ts`) staging `dist/`:
      ...
      verbatim as TypeScript runtime assets. 30 files, 68 kB packed, 246 kB
      unpacked, 300 kB installed.
...
Argus runs whichever engine the project's React Native release ships **by
default** — legacy up to 0.83, V1 from 0.84 — rather than always preferring the
newer one. RN 0.82 and 0.83 pin V1 as well as legacy but ship legacy, and
selecting "V1 whenever V1 is pinned" would test a VM those apps never execute.
`--engine` overrides the choice in either direction.
...
- [x] Follow the RN release's own default engine; expose both as explicit
      opt-in targets (`--engine`).
...
- [x] Document the syntax envelope for both engines, and derive it from real
      binaries rather than prose. `test/hermes-syntax-probe.test.ts` spawns
      whichever VMs are present and checks the policy against them, which is what
      caught two long-standing errors: legacy **runs** `async function` and
      rejects only the arrow form, and V1 rejects `async function*` exactly as
      legacy does, so it is not a superset.
...
- [ ] Cut the remaining `hermes-bin-v*` releases. Only
      `hermes-bin-v250829098.0.16` is published — it covers RN 0.86 and 0.87 and
      nothing else. The prebuilt step 404s on RN 0.83–0.85, and on the date-based
      legacy pins of RN 0.78–0.82 it cannot apply at all (a date ref cannot name
      a release version). Those fall through to the bundled macOS VM where it
      exists, and to `--provision` everywhere else — which is the whole of Linux
      below RN 0.86.
- [ ] **Re-cut the published Linux archives.** The ones on the release page
      require glibc 2.38, so they refuse to start on Ubuntu 22.04 LTS, Debian 12,
      Amazon Linux 2023 and RHEL 9 — the loader rejects them before any Argus
      code runs. The build now targets the oldest supported runner and a gate
      reads the requirement out of the ELF and fails above 2.35, but that landed
      after the release was cut, so the fix is not in users' hands until the
      binaries are rebuilt. musl distributions such as Alpine stay out of reach
      regardless; that is inherent to a glibc build.

---

## Next — test-authoring features

The next milestone. It is deliberately unnumbered: which release these land in
is not decided, and naming one here would be a promise this file cannot keep.
None of them blocked installability; all of them are the reason someone would
ask for the release after this one.
