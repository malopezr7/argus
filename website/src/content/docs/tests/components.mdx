...
description: render, screen, synchronous and async queries, fireEvent and act — real React 19 running inside Hermes.
sidebar:
  order: 6
---
...
The surface is intentionally small. Rendering and events stay synchronous; bounded async
queries cover state that settles through promises or timers.
...
export function Counter() {
  ...
  return (
    <View>
      <Text testID="value">{String(count)}</Text>
...
`react-native` itself is not needed to run this. The bundler aliases it to a small internal
...
result.getByText('Profile'); // queries bound to this render
...
Each of the five predicates comes in six forms:
...
| `findBy*` | waits, then rejects | waits for exactly one |
| `findAllBy*` | waits, then rejects | waits for one or more |
...
```tsx
const result = render(<ProfileLoader />);

const profile = await result.findByText('Ada');
await waitFor(() => expect(profile.props.accessibilityRole).toBe('header'));
await waitForElementToBeRemoved(() => screen.getByText('Loading'));
```

`waitFor(callback, { timeout, interval })`, every `findBy*` / `findAllBy*` query, and
`waitForElementToBeRemoved` use these defaults:

| Option | Default | Meaning |
|---|---:|---|
| `timeout` | `1000` ms | Maximum real wall-clock time |
| `interval` | `50` ms | Requested delay between retries and the basis for the poll budget |
...
<Aside type="caution" title="Hermes timers are a queue, not a clock">
Standalone Hermes exposes `setTimeout`, but both supported engines ignore its delay and
run timers in registration order. A millisecond-only deadline could therefore execute
unbounded retries without advancing `Date.now()`, eventually hitting Argus's per-file
timeout and discarding every result in that file.

Argus applies both the wall-clock `timeout` and a scheduler-turn budget derived from
`ceil(timeout / interval)`. The first exhausted budget stops the wait, and the error names
that budget. This is a deliberate divergence from RNTL's timer-only scheduling model.
</Aside>
...
<Aside type="caution" title="waitFor does not advance fake time">
Unlike RNTL running under Jest, Argus does not auto-advance the fake clock while polling.
A condition driven only by a fake timeout will exhaust its wait even though sibling tests
and the file result remain intact. Advance the clock explicitly before waiting or from the
action that schedules the condition when porting such a suite.
</Aside>
...
The exported helper remains synchronous. Async query utilities manage their own awaited
...
These became live views after v0.2.0. On v0.2.0 — the current release on npm — they are
still snapshots, and a node held across an update silently dispatches into the previous
render's closure: the counter above goes 0 → 1 → 1. Live views land in the next patch
release. If you wrote re-queries to work around that, they stay correct either way; they
are just no longer required once you upgrade.
